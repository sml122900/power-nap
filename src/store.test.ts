...
describe('resolveWidgetModeAction — 홈 위젯 딥링크 진입 판정', () => {
  it('ActiveNap이 있으면 모드 무관하게 항상 alreadyNapping (기존 알람 유지)', () => {
    expect(resolveWidgetModeAction('fast', true)).toEqual({ kind: 'alreadyNapping' });
    expect(resolveWidgetModeAction('slow', true)).toEqual({ kind: 'alreadyNapping' });
    expect(resolveWidgetModeAction('coffee', true)).toEqual({ kind: 'alreadyNapping' });
  });

  it('ActiveNap이 없고 fast/slow면 startNap', () => {
    expect(resolveWidgetModeAction('fast', false)).toEqual({ kind: 'startNap', mode: 'fast' });
    expect(resolveWidgetModeAction('slow', false)).toEqual({ kind: 'startNap', mode: 'slow' });
  });

  it('ActiveNap이 없고 coffee면 openCoffeePanel (새 화면 아님, 기존 인라인 패널)', () => {
    expect(resolveWidgetModeAction('coffee', false)).toEqual({ kind: 'openCoffeePanel' });
  });
});
