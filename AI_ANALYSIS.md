...
> 확정 결정: 주 1회 무료 / 추가 분석 1회 1,000원(인앱결제) / 리포트 + 후속 질문 최대 10턴.
...
- 분석 1회 = 리포트 1개 + 후속 질문 10턴 포함 (턴 추가 구매 상품 없음 — §8 원가 실측 근거로
  판단: 후속 질문 원가가 낮아 유료 연장을 만들면 원가 대비 과다 청구가 되고, 상품이 늘면
  Play 등록·검증 부담만 커진다. 대신 무료 상한 자체를 올렸다)
...
- 후속 질문: 리포트 + 이전 턴을 컨텍스트로 재호출(records_snapshot은 재전송하지 않음 —
  §8 원가가 낮은 이유), 10턴 도달 시 입력창 비활성 + "새 분석을 시작해주세요" 안내
...
- **Phase C — 앱 통합**: 동의 UI, 분석 진입점, 리포트 화면, 후속 질문 10턴
...
- 기존 "3턴 = 비용 방어선"은 데이터 없이 정한 방어적 설계였고, 실측으로 전제가 무너져
  10턴으로 상향(2026-07-17 조사). 근거: 후속 질문 콜은 records_snapshot을 재전송하지
  않아 턴당 원가가 낮음 — 리포트 1회 ≈ 53원, 3턴 풀소진 ≈ 64원, 10턴 추정 ≈ 110원
  (Sonnet 5 도입가 기준). 10턴 상한을 유지하는 이유는 비용이 아니라 컨텍스트 누적에
  따른 답변 품질 저하. 주 1회 무료 분석에도 10턴이 적용되므로 무료 사용자가 늘면 이
  숫자를 재검토할 것(사용자 1,000명이 매주 10턴 소진 가정 시 월 40만원대). 2026-09-01
  표준가 전환 시 1.5배.
