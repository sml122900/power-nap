...
| `home.saveFailedBody` | We couldn't save the alarm, so the one we just scheduled was cancelled. Please try again in a moment. | 신규 추가(§1 감사 후속 — 네이티브 알람은 예약됐는데 로컬 저장이 실패해 방금 예약을 도로 취소한 상황을 안내) — "취소했다"는 사실이 사용자에게 안심 신호로 읽히는지, `scheduleFailedTitle`과 같은 제목을 공유해도 문맥이 헷갈리지 않는지 — **아직 미검수** |
...
| `analysisReport.timeoutError` | 분석 요청이 너무 오래 걸리고 있어요. 네트워크 상태를 확인하고 다시 시도해주세요. | The analysis request is taking too long. Please check your connection and try again. | 신규 추가(AI 분석 로딩 화면이 45초 넘게 걸리면 표시, §7 조용한 실패 감사 후속 조치) — 정중체로 새로 작성해 위 두 줄과 톤이 다름, 통일 여부는 위 항목과 함께 판단 — **아직 미검수** |
...
### 신규 추가 (미검수) — 홈 화면 헤드라인 개편(제목 브랜드 워드마크화)

`home.title`이 "졸리면 그냥 누르세요"에서 "POWER NAP"(브랜드 워드마크, ko/en 동일 —
번역하지 않음)으로 바뀌면서 `home.subtitle`도 새로 작성됐다. 영어는 이번에 새로 작성.

| 키 | 한국어 원문 | 영어 초안 | 검수 포인트 |
|---|---|---|---|
| `home.subtitle` | 최적의 낮잠 시간을 찾는 과정 | Finding your optimal nap time | "과정"(process/journey 뉘앙스)이 영어에서 자연스럽게 읽히는지, 제목 "POWER NAP" 바로 아래 부제로서 톤이 어울리는지 — **아직 미검수** |
