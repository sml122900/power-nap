...
## AI 분석 에러 메시지의 언어 범위 한계 (앱 i18n으로 못 덮는 부분)

`analyze` Edge Function이 401/422/402/409/404 등에서 반환하는 JSON 바디의 `message`
필드(예: "낮잠 기록이 5개 이상 필요하다.", "이번 주 무료 분석을 사용했다...")는 여전히
Edge Function 코드(`supabase/functions/analyze/index.ts`)에 한국어로 하드코딩돼 있다.
`mapInvokeErrorToAnalysisError`가 `body.message`를 우선 신뢰하므로, 앱이 영어 모드여도
이 서버발 에러 메시지들은 한국어로 표시된다 — 위에서 로컬라이즈한 `unknownError`/
`networkError`는 서버가 메시지를 아예 못 보낸 경우(네트워크 단절 등)에만 쓰인다.
이번 작업은 "앱 화면"의 i18n이 범위였고 Edge Function 재수정은 포함하지 않았다 —
서버 에러 메시지까지 로케일 대응하려면 별도 지시로 index.ts에 buildSystemPrompt와
비슷한 `errorMessages(locale)` 매핑을 추가해야 한다.
