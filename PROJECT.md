...
- 배경 brand(6.3과 동일 톤), 명언(`src/missionQuotes.ts`의 `MissionQuote` — `{ text,
  author }`, 기본값 `MISSION_QUOTES` 한/영 각 18개 또는 설정 화면에서 사용자가
  커스텀한 목록)의 `text`를 보여주고 그대로 따라 입력하게 한다. 아래에 `— {{author}}`
  캡션(author 있을 때만). **설정 화면에서 명언을 행 단위로 추가·수정·삭제 가능**
  (사용자 지시, BACKLOG.md "알람 해제 미션" 참고) — 나중에 실존 인물의 유명한 명언을
  출처(author)와 함께 추가할 걸 감안한 구조
