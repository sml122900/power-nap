...
## 6. 화면 명세 (핵심 4개 화면 — prototype.html 참조 + 설정 토글로 켜는 미션 화면 1개)
...
설정에서 켜면 알람 발화 시 6.3(슬라이드/롱프레스 해제) **전에** 이 화면을 먼저
거친다 — 뒷단(6.3~6.4)은 그대로. `useNapWatchdog.resolveNapRoute`가 `Settings.
missionEnabled`와 `ActiveNap.missionCompleted`를 함께 봐서 목적지를 정한다
(BACKLOG.md "알람 해제 미션" 참고).

- 배경 brand(6.3과 동일 톤), 로컬 명언(`src/missionQuotes.ts`, 한/영 각 18개)을
  화면에 보여주고 그대로 따라 입력하게 한다
- 대조는 공백·구두점 제거 + 소문자화 후 비교(오타 하나로 막히지 않게). 건너뛰기 없음
- 3회 연속 실패하면 더 짧은 문구로 교체
- 사운드+진동은 6.3과 마찬가지로 계속 재생(`src/useAlarmPlayback.ts` 공유)
- 하드웨어 뒤로가기 차단(6.3과 동일)
- 테스트 낮잠(`isTest`)은 미션을 건너뛰고 곧장 6.3으로 — 후기를 건너뛰는 것과 같은 이유
