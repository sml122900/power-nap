...
## 5. 데이터 모델 & 학습 로직 (Phase 4-2 — 학습 모델 v2 + 커피냅 3모드)

알람 = 기준시각 + 소요시간. 모드별로 기준시각과 소요시간의 의미가 다르다:
- 일반 낮잠(fast/slow): 알람 = 낮잠 시작 + `TARGET_SLEEP_MIN`(상수 20분) + `latency[mode]`(학습)
- 커피냅(coffee): 알람 = 커피 마신 시각(`coffeeDrankAt`) + `caffeineOnset`(학습)

```ts
type NapMode = 'fast' | 'slow' | 'coffee';  // 바로 잠듦 / 뒤척임 / 커피냅
const TARGET_SLEEP_MIN = 20; // 목표 수면시간(분) — 학습 대상 아님, 고정 상수
...
학습 규칙 (단순하게, ML 없음. fast/slow/coffee 각각 독립 학습):
- 스텝 크기는 모드별 수렴 상태로 분기: 미수렴(converged=false)이면 ±3분, "딱 좋았어요"가 1회라도
  나와 converged=true가 되면 그 이후 ±2분.
- fast/slow: 너무 깊게 잤어요 → `latency[mode]` −step / 딱 좋았어요 → 변화 없음, converged=true /
  아직 부족해요 → `latency[mode]` +step. clamp **min 0, max 20**.
- coffee: 너무 깊게 잤어요 → `caffeineOnset` −step / 딱 좋았어요 → 변화 없음, `converged.caffeine`=true /
  아직 부족해요 → `caffeineOnset` +step. clamp **min 15, max 35** (근거: BACKLOG.md "카페인 발현시간 근거").
- 후기는 그 낮잠의 mode가 가리키는 값에만 반영된다 — 다른 모드는 불변.
- 반영은 즉시 AsyncStorage 저장 → 다음 홈 진입 시 버튼에 표시.
- 후기 화면 보조 경로("직접 조정하기" ±1분 스테퍼 + 숫자 입력)로 제출하면 latency(fast/slow) 또는
  caffeineOnset(coffee)을 절대값으로 clamp해 그대로 반영하고 converged는 건드리지 않는다 — 3버튼
  학습 로직과는 별도 경로.
...
- 마이그레이션: 구형 `{fast, slow}` 또는 `{fast,slow,fastCoffee,slowCoffee}` 오프셋 저장값은
  ...
  에서 재시작한다 — 개념이 아예 달라져 대응되는 값이 없기 때문(사용자 확정 사항).
...
- "낮잠 어땠어요?" + 학습 상태 한 줄("내 수면 대기시간: 잠듦 n분 · 뒤척임 n분" 또는
  커피냅 후기면 "내 카페인 발현: n분") + 3버튼 (각 버튼에 실제 적용될 스텝 크기를 동적
  표시 — 하드코딩 금지, `stepFor` 재사용). 커피냅 후기는 caffeineOnset에 반영됨을 문구로.
- 선택 → 저장 → 홈 복귀 + 토스트(적용된 대기시간/발현시간 안내)
- 보조 경로 "직접 조정하기": fast/slow는 latency(0~20분), coffee는 caffeineOnset(15~35분)을
  숫자 입력 + ±스테퍼로 직접 편집
...
- **Phase 4-3 — 폴리시**: 다크모드, Dynamic Type, reduced-motion, 접근성 라벨, 엣지케이스(권한 거부, 자정 넘김, 후기 미입력 이탈).
