...
- 배경 brand(6.3과 동일 톤), 명언(`src/missionQuotes.ts`의 `MISSION_QUOTES` 기본값,
  한/영 각 18개, 또는 설정 화면에서 사용자가 커스텀한 목록)을 화면에 보여주고 그대로
  따라 입력하게 한다. **설정 화면에서 명언 목록을 직접 편집·추가 가능**(사용자 지시,
