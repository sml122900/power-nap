...
- **위젯 S**(3×2 셀, ~180×110dp): 버튼 1개(바로 잠들 것 같아요)
- **위젯 M**(4×2 셀, ~250×110dp): 버튼 2개(바로/뒤척임), 동일 높이·위계
- **위젯 L**(4×3 셀, ~250×180dp): 버튼 3개 — 바로/뒤척임(상단, 동일 높이) + 커피냅(하단
  전폭, 낮은 띠 — 홈 화면 coffeeBtn과 동일한 위계 낮춤)
- 세 종류 모두 삼성 날씨 위젯처럼 위젯 추가 목록에 별도로 뜨고, 사용자가 원하는 크기를
  골라 배치한다(하나의 위젯이 리사이즈로 변신하는 방식 아님).
- **탭 동작**: 바로/뒤척임 → `powernap:///?widgetMode=fast|slow` 딥링크로 앱이 열리고
