...
- `main`: 네이티브 알람 + 학습 모델 v2 + 커피냅 3모드 + A그룹 + B그룹(풀스크린 인텐트) +
  ...
  AI 분석 후속 질문 JSON 유출 버그 수정 + 후속 질문 상한 10턴 상향 전부 병합 완료.
  **다국어 이후 신규 기능(권한 안내/서버 데이터 삭제/미션/정보 화면/다크모드/
  마이페이지 개편/후속 질문 10턴) 전부 실기기 검증 대기**, 그 이전 기능은 실기기
  검증 완료. `analyze` Edge Function은 `main` 기준으로 재배포 완료(version 10,
  2026-07-18) — 실서버 통합 테스트로 JSON 유출 없음 + 10턴 정상/11턴째 409 재확인.
...
- `mypage-polish`: `main`에 병합 완료(2026-07-18) — 더 이상 별도로 갈 일 없음(정리
  ...
  규칙 추가).
- `followup-10turns`: `main`에 병합 완료(2026-07-18) — 더 이상 별도로 갈 일 없음
  (정리 대상, 삭제는 사용자 지시 시). 후속 질문 유료 연장(1,000원/3턴)은 원가 실측
  결과(3턴 ≈ 18~26원) 근거로 만들지 않기로 결정 — 대신 무료 턴 상한을 3→10으로
  상향. `append_followup_turn` 함수 WHERE절뿐 아니라 `analyses.followup_turns_used`
  컬럼 자체의 CHECK 제약(migrations/0001)도 3으로 박혀 있었던 걸 처음엔 놓쳐
  4턴째부터 계속 409가 났다 — 두 지점 모두 10으로 올리는 마이그레이션(0005 함수,
  0006 컬럼 제약)으로 해결(CLAUDE.md 지뢰 목록에 기록). 답변 전송 중 버튼 문구
  "전송 중" → "답변중"(ko)/"Sending" → "Answering"(en), 턴 소진 시 "새 분석을
  시작해주세요" 안내 추가.
  **⚠️ 배포 충돌 사례**: 이 브랜치가 `main` 기준으로 분기해 `mypage-polish`의 JSON
  유출 수정이 없는 상태에서 `supabase functions deploy`를 실행 — 배포된
  `analyze`(version 9)가 JSON 유출 버그를 되살렸었다(2026-07-17, 병합 전 소스
  검토로 확정, 라이브 호출 1회는 우연히 정상 응답이 나와 착시를 일으킴). `main`에
  `mypage-polish` → `followup-10turns` 순서로 병합한 뒤 `main`에서 재배포(version
  10)로 해결. 실서버 통합 테스트(7개 전부 통과, 10턴 정상 + 11턴째 409) + 자유
  텍스트 응답 3건 수동 확인(JSON 형태 아님)으로 최종 확인. CLAUDE.md 지뢰 목록에
...
- [ ] expo/expo-audio 등 9개 패키지가 업스트림 patch 버전보다 뒤처짐(2026-07-23,
      `expo-doctor` 확인). 기능 문제 아니고 업스트림이 새 패치를 낸 것. v1 실기기 검증이
      현재 조합에서 이뤄졌으므로 출시 전 범프하지 않는다 — 범프하면 알람·결제 회귀
      재검증 필요. v1.1 작업 시작 시 `expo install --fix`로 처리할 것
...
- [ ] 출시 전 체크리스트: `src/config.ts`의 `SHOW_TEST_BUTTONS`를 `false`로(CLAUDE.md
      ...
      다이얼로그(+ Android 설정 딥링크)를 추가해 수정 완료. **실기기 실증(Galaxy
      S24+, Android 14/API 34, 2026-07-18) 결과**: 시스템 설정에 "알람 및 리마인더"
      토글 자체가 없음을 확인 — `USE_EXACT_ALARM`이 자동 부여돼 이 OS 버전에서는
      `SecurityException` 경로가 애초에 트리거될 수 없다는 실증(PROJECT.md §4 참고).
      Android 12/12L(토글을 끌 수 있는 유일한 구간)은 이 기기로 재현 불가 — 코드의
      try/catch 방어로 커버되는 것으로 판단하고 잔여 리스크로만 기록.
...
- **홈 화면 위젯 3종(S/M/L) 구현**(`main` 기준, 아직 별도 커밋 전) — 상세는
  `docs/daily/2026-07-21.md`, 설계 결정은
  `docs/decisions/home-screen-widgets-static-deeplink.md`, jest 관련 트러블슈팅은
  `docs/troubleshooting/reanimated-v4-worklets-jest-import-crash.md` 참고:
  - `plugins/withHomeScreenWidgets.js` 신규 config plugin — 레이아웃/위젯정보 XML,
    바닐라 `AppWidgetProvider`+`RemoteViews`(`NapWidgetProviders.kt`, 서드파티 위젯
    라이브러리 없음), 매니페스트 receiver 3개, `values`/`values-ko` strings.xml을
    전부 코드로 생성. 기존 네이티브 패치 플러그인 3개와 파일 겹침 없음.
  - 헤드리스 JS는 프로세스 킬 상태 신뢰성 미검증 + 삼성 배터리 최적화 리스크로
    조사 단계에서 기각, 딥링크(`powernap:///?widgetMode=...`, 기존 scheme 재사용)로
    앱을 열어 기존 JS 알람 로직(`startFastSlow`/커피냅 인라인 패널)을 그대로 재사용.
  - `resolveWidgetModeAction`(순수 함수, `src/store.ts`) + `handleWidgetModeEntry`
    (의존성 주입 모듈 함수, `app/index.tsx`) — 재탭 가드는 `getActiveNap()` 독립
    확인. 위젯 전용 새 알람 예약 경로 없음.
  - Kotlin 컴파일 에러(`public` 서브클래스가 `internal` 슈퍼클래스 상속 불가) 발견·
    수정, `gradlew assembleDebug` 재실행해 빌드 성공 확인.
  - jest 검증 중 Reanimated v4가 테스트 **import** 시점에 크래시하는 문제 발견 —
    `HomeScreen` 렌더 대신 `handleWidgetModeEntry` 직접 단위테스트로 우회.
  - 검증 4종(tsc/expo-doctor/expo export/jest) + `gradlew assembleDebug` 실컴파일 +
    `aapt dump xmltree`로 최종 APK의 receiver 3개 `exported="true"` 확인 — 전부
    통과. **실기기 배치·딥링크 콜드스타트·재탭가드 검증은 세션 내내 adb 기기
    미인식(3회)으로 보류**, 기기 연결되는 대로 진행 후 통과 시 위젯 브랜치 커밋 →
    `main` 병합 예정.
  - 문서: `PROJECT.md` §6.1.5, `docs/decisions/home-screen-widgets-static-deeplink.md`,
    `REVIEW_NEEDED.md`(위젯 네이티브 문자열 6종, 영어 검수 대기) 전부 반영.
  - **다음**: 위젯 실기기 체크리스트 → 병합 → 릴리즈 검증(`SHOW_TEST_BUTTONS=false`/
    `REVENUECAT_STORE='play'` 전환, 현재는 위젯 검증용으로 `true`/`'test'` 유지
    중) → 체크리스트 H(커피냅/잠금화면/무음모드) 실기기 확인, 전부 기기 연결 대기.

- **홈 화면 위젯 실기기 체크리스트 통과 + `main` 병합**(2026-07-22, `home-screen-widgets`
  브랜치 → `main`, HEAD `420796b`) — USB 디버깅 연결 후 진행:
  - 실기기 검증 중 M/L 위젯이 "위젯을 추가할 수 없습니다" 에러로 렌더링 실패하는
    버그 발견 — RemoteViews가 `<View>`(spacer 용도)를 인플레이트 못 함
    ("Class not allowed to be inflated android.view.View"). `layout_marginEnd`/
    `layout_marginBottom`으로 대체해 수정, 재빌드 후 정상 렌더링 확인.
  - 체크리스트 전부 통과: S/M/L 배치, fast/slow/coffee 탭 동작, 콜드스타트 딥링크,
    재탭가드(알람 시각 불변 확인), 위젯 카드 가독성. Task H도 병행 확인 —
    10초 테스트 알람으로 무음모드(STREAM_ALARM만 안 뮤트, RING/NOTIFICATION은
    뮤트됨 확인)·잠금화면 자동 점등(풀스크린 인텐트) 실증.
  - 디버그 빌드가 NordVPN + NAT 헤어핀 문제로 Metro에 연결 못 해 무한 로딩 —
    release variant(`expo run:android --variant release`, JS 번들 내장)로 전환해
    우회(디버그 dev-client는 실기기 자동화 테스트에 부적합할 수 있음, 참고용).
  - `expo prebuild --clean` 재실행 과정에서 `app.json`에 없던 `android.versionCode`가
    자동으로 추가된 것 발견 — 위젯과 무관해 `chore: set android versionCode for
    first release` 커밋으로 분리.
  - 커밋 4개(관심사별 분리: versionCode / 위젯 플러그인+레이아웃 / 앱 딥링크 배선 /
    문서) → 검증 4종 통과 → `main` fast-forward 병합 → 브랜치 삭제(`sleep-character`는
    보존). CLAUDE.md 지뢰 2건 추가(RemoteViews `<View>` 제한, prebuild가 app.json
    필드를 조용히 추가할 수 있음).
  - **다음**: 릴리즈 검증(`SHOW_TEST_BUTTONS=false`/`REVENUECAT_STORE='play'` 전환),
    push 여부 확인 대기.

- **첫 실행 온보딩(4장 튜토리얼) 구현 + 실기기 검증 통과**(2026-07-22, `onboarding`
  브랜치, `main` `420796b` 기준 분기, 사용자 명시 지시 — 카피 초안 승인 후 구현):
  - 전체화면 슬라이드 4장(3모드 차이/미션·기상루틴/AI 분석 주 1회 무료/위젯), `ScrollView`
    pagingEnabled로 가로 스와이프(별도 캐러셀 라이브러리 없음). 건너뛰기 1~3장,
    4장은 "시작하기". 다크/라이트 테마 따름(`useThemeColors`).
  - `shouldShowOnboarding`(순수 함수, `src/onboarding.ts`) — 완료 여부 +
    위젯 딥링크 진입 + 진행 중이던 낮잠/설문 유무로 판정, 위젯 액션과 낮잠 복귀가
    온보딩보다 우선(사용자 지시). `app/index.tsx`에 `useNapWatchdog`과 독립적으로
    1회 판정하는 effect 추가 — 두 훅이 동시에 `router.replace`를 부르는 경우가
    구조적으로 없음(라우팅 타겟이 서로 배타적).
  - 설정 화면에 "둘러보기" 섹션("온보딩 다시 보기") 추가, `settings.test.tsx`를
    7섹션 기준으로 갱신.
  - 온보딩 4장 위젯 카피 초안에 사실 오류 있었음("앱을 안 열고도 시작") — 사용자가
    지적해 재작성("앱이 열리면서 알람까지 바로 맞춰진다"). 영어 2장 "harder to
    snooze" 표현도 스누즈 기능이 없어 오해 소지 있다는 지적으로 REVIEW_NEEDED에
    재검토 표시해둠.
  - **실기기 검증 전부 통과**: 앱 데이터 삭제 후 첫 실행 자동 노출, 4장 스와이프+
    인디케이터, 건너뛰기/시작하기 → 홈 이동 + 재실행해도 재노출 안 됨(플래그 저장
    확인), 설정 "다시 보기" 재생, 위젯 딥링크 우선순위(온보딩 완전히 건너뛰고 수면
    화면 직행 확인), 다크/라이트 테마 렌더링. 검증 중 `uiautomator dump` 시
    git-bash가 `/sdcard/...` 경로를 자동으로 Windows 경로로 mangle하는 문제를
    `//sdcard/...`(더블 슬래시)로 우회하는 법을 확립 — 눈대중 좌표 탭이 계속
    빗나가던 문제 해결.
  - 검증: jest(`shouldShowOnboarding`/`getOnboardingComplete`/`setOnboardingComplete`
    단위테스트 + `settings.test.tsx` 갱신) + tsc + expo export ios 전부 통과.
    라우트 추가 후 `expo start` 1회로 타입 재생성 필요했음(기존 지뢰 재확인).
  - PROJECT.md §6.0, REVIEW_NEEDED.md(온보딩 영어 카피 5건) 반영.

- **낮잠 모드 라벨 개편**(2026-07-22, `onboarding` 브랜치, 사용자 명시 지시) —
  기존 두 표현 체계(홈 버튼의 긴 예측형 문장 "바로 잠들 것 같아요"/"좀 뒤척일 것
  같아요"와 히스토리·설정 등의 짧은 표현 "바로 잠듦"/"뒤척임")를 "매우 졸림"/
  "조금 졸림" 하나로 통일 — 앱(홈 버튼·학습 상태 문구·설문 상태 문구)·위젯(네이티브
  strings.xml)·온보딩(슬라이드1)·설정(알람 시간 조정 행 라벨)·히스토리·AI 분석
  제안 라벨까지 전부. `common.napMode.fast/slow`가 히스토리·후기 화면이 공유하는
  단일 소스라 그것만 바꿔도 두 화면이 같이 갱신됨(`common:napMode.${mode}` 참고).
  영문("Very sleepy"/"A bit sleepy")은 이번에 새로 작성, REVIEW_NEEDED에 등록.
  tsc/jest(22 suites, 209 tests) 전부 통과.

- **릴리즈 설정 전환**(2026-07-22 23:38, `main` 직접 작업, 커밋 `255f75d`) —
  `src/config.ts`의 `SHOW_TEST_BUTTONS=false`(개발용 단축/더미 버튼 숨김, 사용자
  대상 "10초 알람 체험"은 그대로 노출)와 `REVENUECAT_STORE='play'` **이미 전환
  완료**. Play Console DUNS 계정은 아직 미승인이라 실결제는 안 되지만, RevenueCat
  Test Store 키로 릴리즈 빌드를 실행하면 SDK가 앱을 강제 종료시키는 것(CLAUDE.md
  지뢰 목록)을 피하기 위해 먼저 전환해둔 것 — DUNS 승인 전까지 사용자가 결제
  버튼을 누르면 구매가 완료되지 않는 상태임을 인지할 것(§9 감사에서 재확인).

- **릴리즈 서명 수정 + 홈 화면 헤드라인 개편**(2026-07-23, `main` 직접 작업,
  커밋 `93a29ba`/`7cfca21`/`ff46722`):
  - 릴리즈 APK가 `CN=Android Debug`로 서명되고 있던 문제를 실기기 검증 중
    `apksigner verify --print-certs`로 직접 발견. 새 release keystore
    (`C:\Users\sm553\keys\power-nap-release.keystore`, alias `powernap-release`,
    리포 완전히 바깥에 위치)를 생성하고 `plugins/withReleaseSigning.js`(신규
    config plugin)로 `android/` 재생성 시마다 서명 설정이 자동 복원되게 만듦.
    경로/별칭/비밀번호는 `.env`의 non-`EXPO_PUBLIC_` 변수 4개로 관리, 하나라도
    비면 prebuild 자체가 실패. 재빌드 후 `CN=SungMin-Lee, OU=lifebook` 서명
    확인, 기존 debug-서명 앱 제거 후 재설치 완료 — **현재 실기기 설치본이
    최종 AAB 후보와 동일한 서명키·홈 카피를 반영한 상태**.
    상세: `docs/decisions/release-signing-config-plugin.md`,
    `docs/troubleshooting/release-apk-signed-with-debug-key.md`.
  - 홈 화면 제목 "졸리면 그냥 누르세요" → "POWER NAP"(브랜드 워드마크, ko/en
    동일), 부제 → "최적의 낮잠 시간을 찾는 과정"(사용자 확정 문구, en 초안
    REVIEW_NEEDED 등록). 제목 fontSize 28→36(기존 `fontFamily.heavy`가 이미
    번들 최대 굵기라 "더 두껍게"는 사이즈 확대로만 구현 — Black 폰트 에셋 없음을
    사용자에게 알림). 다크/라이트 실기기 확인, 레이아웃 밀림 없음.
  - 실기기 위젯에 낮잠 모드 라벨 개편(76c1c02) 이전 문구가 남아있다는 보고 →
    플러그인 소스/`android/` 생성물/릴리즈 APK 문자열(aapt2) 순으로 확인해 셋
    다 정상임을 확인, 진짜 원인은 `expo run:android`가 `android/`가 이미
    있으면 config plugin을 재적용하지 않는다는 것(코드 문제 아님, 이 세션의
    서명 작업으로 `expo prebuild --clean`을 이미 돌려 부수적으로 해결됨).
    CLAUDE.md 지뢰 기록, 코드 수정 없음.
    상세: `docs/troubleshooting/stale-widget-labels-after-prebuild-skip.md`.
  - expo-doctor 확인 중 9개 패키지의 patch 버전 드리프트 발견 — v1 실기기
    검증이 현재 조합에서 이뤄졌으므로 지금 안 고침, "미해결 항목"에 기록
    (v1.1 시작 시 `expo install --fix`).
  - 검증 4종(tsc/expo-doctor 기존 드리프트 1건 제외 통과/expo export ios/jest
    209개) 통과, 실기기 다크·라이트 확인.
  - **다음**: 위젯을 실기기에 새로 추가해 라벨 최종 확인(이번 세션 uninstall로
    기존 위젯 인스턴스는 사라진 상태), Play Console 계정(DUNS) 발급 후 이번에
    만든 keystore로 서명한 AAB 실제 업로드.

- **v1 AAB 제출 직전 전수 감사 + 발견 사항 반영**(2026-07-24, `main` 직접 작업,
  미커밋 — 관심사별 커밋 예정):
  - 9개 섹션(시크릿 노출/릴리즈 설정/권한 전수/config plugin/알람 불변식/조용한
    실패/죽은 코드/서버 정합성/개인정보·문서 정합성) 병렬 감사 완료. 시크릿 6종
    번들·git 히스토리 미노출, config plugin 5종 전부 loud-fail·멱등성 확인,
    Edge Function 3개 전부 배포본=`main` 소스 일치 등 대부분 클린. 발견된 P0/P1은
    아래에서 처리.
  - **[P0] release APK에 `SYSTEM_ALERT_WINDOW` 권한 + `DevSettingsActivity`
    혼입 — 근본 원인 규명 후 수정**: `gradlew clean assembleRelease` 클린
    재빌드 + `aapt2 dump permissions`/`manifest-merger-blame-release-report.txt`로
    실증. 최초 가설("release에 debug AAR 혼입")은 틀렸음 — 실제로는 서로 다른
    두 원인: (1) `SYSTEM_ALERT_WINDOW`는 `@expo/config-plugins`의 기본
    AndroidManifest 템플릿이 매 `prebuild --clean`마다 넣는 "OPTIONAL
    PERMISSIONS" 기본값(우리 코드는 이 권한을 전혀 안 씀), (2)
    `DevSettingsActivity`는 `expo-alarm-module@1.2.0` 자신의
    `android/src/main/AndroidManifest.xml`에 build-type 구분 없이 하드코딩된
    라이브러리 버그. 신규 config plugin `plugins/withRemoveUnusedDebugEntries.js`
    추가(권한은 `AndroidConfig.Permissions.removePermissions`로 직접 제거,
    액티비티는 `tools:node="remove"`로 매니페스트 병합 단계에서 제외) —
    `expo prebuild --clean` + 재빌드 후 `aapt2 dump permissions`/`dump xmltree`
    양쪽 다 0건 확인, `apksigner verify --print-certs`로 서명(`CN=SungMin-Lee`)도
    재확인. 라이브러리가 이후 버전에서 자체 수정해도 제거 대상이 없어지는 것뿐이라
    무해.
  - **[P0] `PRIVACY_POLICY.md` 시행일 placeholder** — 레포 파일은 `[월] [일]`
    미기재 상태였으나, 배포본(`lifebookapplication.com/privacy/powernap`)은
    이미 "2026년 7월 16일"로 채워져 있었음(레포만 뒤처져 있던 상태) — 레포를
    배포본과 동일하게 맞춤. 날짜 외 본문 차이는 전수 diff는 안 함(WebFetch 요약
    기준 구조 일치만 확인) — 필요하면 별도로 정밀 대조 요망.
  - **[P1] 릴리즈 로깅** — `src/purchases.ts`의 Supabase UID/RevenueCat
    appUserID/거래ID `console.log` 2곳을 `__DEV__` 가드로 감쌈. 그 외 `src/`
    전수 검색 결과 PII 로깅 추가 발견 없음.
  - **[P1] 스플래시+아이콘 재생성** — `assets/icon.png`/`android-icon-background.png`에
    구워져 있던 디자인툴 아트보드 가이드선을 sharp로 재생성해 제거(클린한
    `android-icon-foreground.png`를 소스로 재구성, 배경색 `#E6F4FE`는 기존
    adaptiveIcon 설정과 동일하게 유지). `splash-icon.png`는 그동안 Expo 기본
    템플릿 placeholder(회색 동심원)가 그대로 남아있던 것을 발견 — 같은 클린
    소스에서 알파 마스크만 추출해 흰색 실루엣으로 재생성, `app.json`의
    `expo-splash-screen` 플러그인에 설정 객체(`image`/`imageWidth: 180`/
    `resizeMode: contain`/`backgroundColor: #4353E0`/`dark.backgroundColor:
    #12172A`)를 새로 배선(기존엔 설정 객체 자체가 없어 이미지가 연결 안 되고
    있었음). 스토어 등록용 `assets/store-icon-512.png`(512×512, 같은 소스)도
    같이 생성. 이미지 처리에 필요한 sharp는 스크래치패드에만 설치, 레포
    `package.json`은 안 건드림.
  - **[P1] `saveActiveNap` 유령 알람 (알람 경로 — 이번 세션 유일한 알람 코드
    변경)**: `app/index.tsx`의 `startFastSlow`/`startCoffeeNap` — 네이티브
    알람 예약 성공 후 `saveActiveNap`(AsyncStorage) 실패 시 예외가 안 잡혀
    취소 불가능한 알람이 남던 문제. `handleWidgetModeEntry`와 같은 패턴으로
    `saveActiveNapOrRollback`(컴포넌트 바깥 순수 함수, deps 주입)을 추출 —
    실패 시 `cancelAlarmNotificationAsync(notificationId)` 호출 후 사용자에게
    알림(`home.saveFailedBody` 신규 키, REVIEW_NEEDED 등록). 정상 경로는 항상
    `saveActiveNap` 성공이라 회귀 없음 — `src/saveActiveNapOrRollback.test.tsx`
    (신규 3케이스: 성공/실패+취소호출/notificationId=null이어도 취소 호출)로
    확인.
  - **[P1] 사용자가 갇히는 화면 3건**: `app/onboarding.tsx`(완료 플래그 저장
    실패해도 홈으로 나감), `src/WakeRoutineScreen.tsx`(체크리스트 저장 실패해도
    다음 화면으로 — 하드웨어 뒤로가기가 막혀있어 이전엔 진짜로 못 나갔음),
    `app/analysis.tsx`(로딩 화면에 닫기 버튼 추가 + 45초 타임아웃 시 에러
    화면으로 전환, `analysisReport.timeoutError` 신규 키). 나머지 조용한 실패
    (설정 토글·스테퍼·명언 삭제)는 BACKLOG로 넘김(사용자 지시) — 근본 대책은
    `src/store.ts` AsyncStorage 쓰기 공통 래퍼, 아직 미착수.
  - **문서 드리프트 정정**: BACKLOG.md의 홈 위젯/AI 분석 섹션이 이미 출시된
    기능을 "미착수"로 서술하던 것 정정(취소선 + 실제 상태 표기).
    STATUS.md에 누락돼 있던 `255f75d`(릴리즈 설정 전환) 항목을 시간순 위치에
    추가. PROJECT.md §5 데이터 모델을 `src/store.ts` 실제 필드(missionEnabled,
    wakeRoutineEnabled, isTest, isPreview, alarmDismissed, NapRecord.isTest/
    wakeChecklist, manualAdjust.source의 'ai-analysis')와 동기화, §7 MVP
    스코프 절엔 "최초 스냅샷" 안내를 추가(내용 자체는 보존).
  - **`sleep-character` 브랜치**: 되돌려진 서버 버그(follow-up 3턴 제한, JSON
    유출)를 포함한 채 미병합 상태로 남아있어 실수로 그 브랜치에서 deploy하면
    재발할 위험 — 삭제 권장, 커밋 해시(`2b43ee7`)는 BACKLOG.md "보류 — 잠자는
    강아지 캐릭터" 섹션에 기록해 복원 가능하게 해둠. **브랜치 삭제 자체는
    사용자 확인 전 실행 안 함**(원격 브랜치라 공유 상태 변경).
  - 검증: tsc/jest(23 스위트, 212개, 신규 3개 포함 전부 통과)/expo export ios/
    expo-doctor(기존 9개 패키지 patch 드리프트 외 이상 없음) + `expo prebuild
    --clean` → `gradlew clean assembleRelease` 클린 재빌드 2회(P0 수정 전/후
    비교) 전부 통과.
  - **다음**: 실기기(adb 미연결로 이번 세션엔 불가) — 스플래시/아이콘 확인,
